...
def calculate_moves(grid, player, opponent, players_chance, default_value):
    ...
    if players_chance:
        ...
        if value!=False:
            grid[value[0]][value[1]] = player
            twin, tloss = calculate_moves(grid, player, opponent, not players_chance, default_value)
            winnings+=twin
            losses+=tloss
            grid[value[0]][value[1]] = default_value
            has_played = True
    if not players_chance:
    ...
    for i in range(3):
        for j in range(3):
            if grid[i][j]==default_value:
                ...
                print(winnings, losses, end='|')
                # print(round((winnings/losses)*100,2), end='|')
